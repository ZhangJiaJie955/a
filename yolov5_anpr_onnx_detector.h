...
public:
  ...
  //**************************
	...
	* @return void
	...
	void detect_lpn(const cv::Mat& frame, std::vector<int>& classIds,
	std::vector<float>& confidences, std::vector<cv::Rect>& boxes, float& confidence_lpn_roi, cv::Rect& lpn_roi,
	float confThreshold, float nmsThreshold);
	void evaluate_lpn_with_lpn_detection(cv::Mat& frame, cv::Mat& defimg, std::string& lpn, std::list<float>& confidences, std::list<int>& classes,std::list<cv::Rect>& boxes);
};
