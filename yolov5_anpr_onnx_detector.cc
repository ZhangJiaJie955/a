...
bool load_onnx(const std::string& dir_path, std::string& yolo_carac_detect_model_file) {
	...
	std::experimental::filesystem::path p(dir_path);
	if (std::experimental::filesystem::exists(p) && std::experimental::filesystem::is_directory(p))    // does p actually exist?
	{
		std::experimental::filesystem::directory_iterator end_itr; // default construction yields past-the-end
		for (std::experimental::filesystem::directory_iterator itr(p); itr != end_itr; ++itr)
		{
			if (std::experimental::filesystem::is_regular_file(itr->status()))
			{
				try {
					const std::experimental::filesystem::path copie(itr->path());
...
float Yolov5_anpr_onxx_detector::detect(const std::string& dir, const bool show_image, const int time_delay)
{
	...
	while (it_image_filenames != image_filenames.end())
	{
		...
		std::experimental::filesystem::path p_(*it_image_filenames);
...
{
	std::experimental::filesystem::path p_(image_filename);
	if (exists(p_) && std::experimental::filesystem::is_regular_file(p_))    // does p actually exist?
	{
		int flags = -1;//as is
...
/** @brief Given the @p input frame, create input blob, run net and return result detections and from detections extract lpn. Difference between this func and detect func is that
	evaluate_lpn_with_lpn_detection first focus on license plate detection and secondly on its characters. This is supposed to get best results
		 ...
		 *  @param[out] boxes : A set of bounding boxes.
		 */
void Yolov5_anpr_onxx_detector::evaluate_lpn_with_lpn_detection(cv::Mat& frame, cv::Mat& defimg, std::string& lpn,
	 std::list<float>& confidences, std::list<int>& classes,std::list<cv::Rect>& boxes)
{
	if (frame.rows > 0 && frame.cols > 0) {
		...
		if (mean_[0] > 2.0f && mean_[0] < 250.0f) {
			...
			cv::Rect lpn_roi;
			const float confThreshold = 0.25f; float nmsThreshold = 0.2f;
			...
			if (true) {
				...
				if (false) {
					...
					filter_iou(classIds,vect_of_confidences,vect_of_detected_boxes, nmsThreshold);
					if (vect_of_detected_boxes.size() > 2) {
						...
						while (it_out_classes_ != tri_left_classIds.end()
							...
							if ((*it_out_classes_ < class_lpn) || (lpn.empty())) {
								...
								cv::rectangle(defimg,box_in_global_image,cv::Scalar(0,0,255),3,1,0);
							}
							it_out_classes_++;
							it_confidences++;
							it_boxes++;
